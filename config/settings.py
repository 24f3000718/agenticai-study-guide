...
class ContentType(str, Enum):
    ...
    SYLLABUS = "Syllabus"                  # syllabus documents mapping curriculum topics
...
# =============================================================================
# CS Subject Keywords for Topic Detection
# =============================================================================
...
# (Exam-specific patterns removed — this workspace only uses the CS subject track)
...
class APIKeyManager:
    ...
    GROQ_API_KEY_1, GROQ_API_KEY_2, GROQ_API_KEY_3, GROQ_API_KEY_4, GROQ_API_KEY_5, etc.
    """
    ...
    def __init__(self, key_prefix: str = "GROQ_API_KEY"):
        ...
        if not self.keys:
            raise ValueError(
                f"No API keys found with prefix '{key_prefix}'. "
    ...
    def _load_keys_from_environment(self) -> None:
        ...
        """
        # Try the base key without number (for backward compatibility)
        base_key = os.getenv(self.key_prefix)
        if base_key and base_key.strip():
            key_value = base_key.strip()
            ...
            print(f"[APIKeyManager] Loaded base key: {self._mask_key(key_value)}")
        
        # Try numbered keys: GROQ_API_KEY_1, GROQ_API_KEY_2, ... GROQ_API_KEY_5, etc.
        index = 1
        while True:
            ...
            print(f"[APIKeyManager] Loaded key {index}: {self._mask_key(key_value)}")
            index += 1
        ...
        if keys_string:
            for key in keys_string.split(","):
                ...
                if key and key not in self.keys:
                    ...
                    print(f"[APIKeyManager] Loaded key from GROQ_API_KEYS: {self._mask_key(key)}")
        
        print(f"[APIKeyManager] Total API keys loaded: {len(self.keys)}")
    ...
    def mark_key_failure(self, key: str = None) -> None:
        ...
        if key in self.key_status:
            ...
            # Use config.MAX_RETRIES_PER_KEY from the global config
    ...
    def rotate_to_next_key(self) -> str:
        ...
        # All keys marked as failed - reset and try again
    ...
    def get_key_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_key_masked": self._mask_key(self.get_current_key()) if self.keys else None,
            "keys": [
                {
...
# =============================================================================
# Track Display Names and Descriptions
# =============================================================================
...
TRACK_DESCRIPTIONS: Dict[TrackType, str] = {
    ...
    **Exam Preparation Assistant**
    ...
    - Topic-wise question practice with solutions drawn from study materials
