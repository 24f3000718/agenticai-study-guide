...
def render_mode_selector():
    """
    Render the study/exam/quick mode selector.
    """
    ...
    with col2:
        if st.button("Exam Mode", type="secondary", use_container_width=True):
            ...
            st.rerun()
    with col3:
        if st.button("Quick Revision", type="secondary", use_container_width=True):
            st.session_state.current_mode = "Quick Revision"
            st.rerun()

    mode = st.session_state.get("current_mode", "Study Mode")
    mode_descriptions = {
        "Study Mode": "Comprehensive explanations with theory, examples and source attribution.",
        "Exam Mode": "Step-by-step solutions formatted for exam submissions.",
        "Quick Revision": "Concise bullet-point answers for rapid last-minute review.",
    }
...
def generate_response(query: str) -> Dict[str, Any]:
    """
    Generate a response from the active track, respecting the selected mode.
    """
    track = st.session_state.get("current_track")
    if not track:
        return {
    ...
    # map UI mode label → RAG chain mode string
    mode_map = {
        "Study Mode": "study",
        "Exam Mode": "exam",
        "Quick Revision": "quick",
    }
    current_mode = st.session_state.get("current_mode", "Study Mode")
    chain_mode = mode_map.get(current_mode, "study")

    try:
        response = track.process_query(query, query_type=chain_mode)
        if isinstance(response, dict):
            return response
        return {
            "answer": str(response),
            "track_type": track.track_type.value if hasattr(track, "track_type") else "unknown",
            "sources": []
        }
