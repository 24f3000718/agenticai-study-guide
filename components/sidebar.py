...
from components.export_manager import render_export_section  # download buttons for chat/notes
...
def render_api_key_status():
    ...
    try:
        ...
        # create expandable section with summary (no individual key values shown)
        with st.expander("Key Summary"):
            ...
            # For security, do not expose per-key values in the UI.
            st.caption("Individual API key values are hidden for security reasons.")
...
def render_sidebar():
    ...
    with st.sidebar:  # everything inside this block goes in the sidebar
        ...
        # export section - download chat history and study notes
        render_export_section()

        # api key status section - shows at bottom of sidebar
        render_api_key_status()

# ---- added: wire export manager into sidebar ----
# Call this at the bottom of render_sidebar() to show download buttons.
# Already imported via render_sidebar if you add the import below.
